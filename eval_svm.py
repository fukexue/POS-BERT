...
# import pct_office as pcts
from dataloader import ModelNet40_STRL, DataAugmentationDINO, DataAugmentationSSTest
...
def extract_feature_pipeline(args):
    ...
    # elif args.arch in pcts.__dict__.keys():
    #     model = pcts.__dict__[args.arch]()
...
def cal_singular(feats):
    from numpy import linalg as la
    feat_mean = feats.mean(0, keepdims=True)
    feat_sub = feats-feat_mean
    C = np.dot(feat_sub.transpose(1,0), feat_sub)/feat_sub.shape[0]
    U, sigma, VT = la.svd(C)
    return sigma

def plot_sigma_fig(sigma):
...
if __name__ == '__main__':
    ...
    sigma_sort = cal_singular(test_features.cpu().numpy())
    ...
    if utils.get_rank() == 0:
        ...
        print_info = f"SVM classifier Acc: {top1}"
        print(print_info)
    dist.barrier()
