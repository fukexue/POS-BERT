...
from utils_3d.pct_util import sample_and_group
...
class PCT(nn.Module):
    def __init__(self, samplepoint=256, embed_dim=256, depth=2, num_heads=4, mlp_ratio=2.0,
                 ...
                 norm_layer=nn.LayerNorm, mask_type='rand', mask_ratio=[0.2,0.4], masked_im_modeling=False, **kwargs):
    ...
    def prepare_tokens(self, x):
        ...
        new_xyz, new_feature = sample_and_group(npoint=int(np.ceil(x.size(1)/self.samplepoint)), radius=None, nsample=self.samplepoint, xyz=xyz, points=x)
        ...
        if self.mask_type=='rand':
            ...
            bool_masked_pos = self._mask_center(x.transpose(1, 2), self.masked_im_modeling)
        else:
            print('mask_type must in [rand, block]')
    ...
    def forward(self, x):
        ...
        if not math.isfinite(torch.sum(x)):
            print("pct last output is {}, stopping training".format(torch.sum(x).item()), force=True)
            sys.exit(1)
