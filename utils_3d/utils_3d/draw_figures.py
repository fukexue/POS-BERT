...
def show_pointcloud_2part(pc0, pc1, size=10, theta_1=0, theta_2=0, c0='#1f4f89', c1='#b30000', savename=None, close=False):
    if type(pc0) == torch.Tensor:
        pc0 = pc0.detach().cpu().numpy()
    if pc0.shape[0]==3:
        pc0 = pc0.transpose()
...
def validate_vis(P1_gt, P2_gt, pair_label_gt):
    # check the corresponding between P1 and P2
    # P1_gt: Nx3
    # pair_label_gt: N
    if type(P1_gt) != torch.Tensor:
        P1_gt = torch.from_numpy(P1_gt)
    if type(P2_gt) != torch.Tensor:
        P2_gt = torch.from_numpy(P2_gt)
    if type(pair_label_gt) != torch.Tensor:
        pair_label_gt = torch.from_numpy(pair_label_gt)
    overlapped_idx = pair_label_gt.nonzero().squeeze()
    N1 = P1_gt.shape[0]
    N2 = P2_gt.shape[0]
    c0 = torch.tensor([255, 0, 0]).unsqueeze(0).repeat([N1, 1])
    c1 = torch.tensor([0, 255, 0]).unsqueeze(0).repeat([N2, 1])
    plotly_show_2pointcloud(P1_gt[:, :3] / 10, P2_gt[:, :3] / 10, c0=c0[:], c1=c1[:])

    overlapped_idx = overlapped_idx[:20]
    c0 = torch.tensor([255, 255, 255]).unsqueeze(0).repeat([N1, 1])
    c1 = torch.tensor([255, 255, 255]).unsqueeze(0).repeat([N2, 1])
    c0[overlapped_idx] = torch.tensor([0, 0, 255])
    c1[overlapped_idx] = torch.tensor([255, 0, 0])
    plotly_show_2pointcloud(P1_gt[:, :3] / 10, P2_gt[:, :3] / 10, c0=c0[:], c1=c1[:])
...
def show_pointcloud_N(pc, color='b', size=1, savename=None, close=False, isshow=False, format='svg'):
    if type(pc) == torch.Tensor:
        pc = pc.detach().cpu().numpy()
    if pc.shape[1]==3:
        pc = pc.transpose(0,2,1)
    B,N,C = pc.shape
    fig = plt.figure()
    B_sqrt = int(np.sqrt(B))
    B = B_sqrt*B_sqrt
    color_flag = False
    for i in range(B):
        if color=='coor' or color_flag:
            pc_nomin = (pc[i] - np.min(pc[i], axis=0))
            color = pc_nomin/np.max(pc_nomin, axis=0)
            color_flag = True
        ax = fig.add_subplot(B_sqrt, B_sqrt, i+1, projection='3d')
        ax.scatter(pc[i, :, 0], pc[i, :, 1], pc[i, :, 2], s=size, alpha=0.5, color=color)
        plt.axis("off")
        ax.view_init(0,0)
    if isshow:
        plt.show()
    if savename is not None:
        plt.savefig(savename, format=format, bbox_inches='tight', transparent=True, dpi=600)
        if close:
            plt.close(fig)
