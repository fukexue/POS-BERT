...
from models.model_seg import POS_BERT, cal_loss
from git_log import get_sha
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='pco-bert', help='model name')
    ...
    parser.add_argument('--model_path', type=str, default='../weight/pcobert_pretrain.pth', help='ckpts')
...
def main(args):
    ...
    # shutil.copy('models/.py', str(exp_dir))
    # shutil.copy('models/pointnet2_utils.py', str(exp_dir))

    criterion = cal_loss(smoothing=False).cuda()
...
## we use adamw and cosine scheduler
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in enumerate(trainDataLoader):
            ...
            seg_pred = model(points, to_categorical(label[:,0], num_classes)).permute(0, 2, 1)
        ...
        if test_metrics['class_avg_iou'] > best_class_avg_iou:
            ...
            savepath = str(checkpoints_dir) + '/best_cls_model.pth'
...
def test(args):
    ...
    print('\nEval: Class avg mIOU: %f   Inctance avg mIOU: %f' % (test_metrics['class_avg_iou'], test_metrics['inctance_avg_iou']))
